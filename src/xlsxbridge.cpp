...
sheet_names = {'DOMACE': 'DOMAĆE SERIJE', 'STRANE': 'STRANE SERIJE'}
...
cols = ['#', 'NAZIV SERIJE', 'KATEGORIJA', 'GODINA', 'VELIČINA',
        'HARD DISK', 'IMDB OCJENA', 'ŽANR', 'GLEDANO']
...
for cat, items in groups.items():
    ...
    for row_i, item in enumerate(items, 2):
        ...
        vals = [
            row_i - 1,
            item.get('name',''),
            'Domaća' if item.get('category') == 'DOMACE' else 'Strana',
            item.get('year',''),
            item.get('size',''),
            item.get('hard_disk',''),
...
bool XlsxBridge::runPython(const QString &script, const QStringList &args,
    ...
    tf.close();  // Keep file until process finishes
