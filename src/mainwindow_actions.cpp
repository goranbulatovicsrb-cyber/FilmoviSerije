...
void MainWindow::onImdbResult(ImdbResult result) {
    if (!result.success) {
        statusBar()->showMessage("IMDB: " + result.error, 4000);
        return;
    }
    ...
    // Refresh current selection if it's the same series
    ...
    // Update table row
    for (int r = 0; r < m_model->rowCount(); r++) {
        if (m_model->item(r, COL_ID) &&
            m_model->item(r, COL_ID)->data(Qt::UserRole).toInt() == result.seriesId) {
            // Update rating cell
            auto *ratingItem = m_model->item(r, COL_RATING);
            if (ratingItem) {
                ratingItem->setText(result.rating > 0 ?
                    QString("%1").arg(result.rating, 0, 'f', 1) : "—");
                ...
                if (result.rating >= 7.5) {
                    QFont f; f.setBold(true); ratingItem->setFont(f);
                }
            }
            // Update genre cell
            if (m_model->item(r, COL_GENRE))
    ...
    updateStats();
    statusBar()->showMessage(
        QString("⭐  %1 — IMDB: %2/10").arg(result.title).arg(result.rating, 0,'f',1), 4000);
}
...
void MainWindow::onImdbBatchComplete() {
    ...
    m_imdbStatus->setText("✅  IMDB batch preuzimanje završeno");
    loadAllData();
    QMessageBox::information(this, "Završeno",
        "✅  IMDB ocjene su uspješno preuzete za sve serije!");
}
